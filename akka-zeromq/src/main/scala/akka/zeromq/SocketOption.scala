...
object ZMQMessage {
  ...
  def apply[T](frames: T*)(implicit converter: T ⇒ ByteString): ZMQMessage =
    apply(frames map { f ⇒ converter(f) }: _*)

  implicit val Protobuf2ByteString = (m: Message) ⇒ ByteString(m.toByteArray)
}
